...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        webView.setWebChromeClient(new WebChromeClient() {
            ...
            @Override public boolean onCreateWindow(WebView view, boolean isDialog, boolean isUserGesture, android.os.Message resultMsg){
                // AppSheet suele abrir PDFs/adjuntos con window.open() o target="_blank".
                // Sin esto, esos clics no disparan shouldOverrideUrlLoading y no pasa nada.
                WebView popupCatcher = new WebView(MainActivity.this);
                popupCatcher.setWebViewClient(new WebViewClient(){
                    @Override public boolean shouldOverrideUrlLoading(WebView v, String url){
                        manejarUrlExterna(url);
                        return true;
                    }
                });
    ...
    /**
     * Lógica central de ruteo de URLs. La usan tanto shouldOverrideUrlLoading (navegación
     * normal dentro del webView) como onCreateWindow (cuando AppSheet abre el link con
     * window.open() o target="_blank", que es el caso más común para adjuntos/PDFs).
     * Retorna true si la URL fue interceptada/manejada por la app (no debe navegar el WebView),
     * false si debe dejarse pasar normalmente.
     */
    private boolean manejarUrlExterna(String url){
        ...
        if(url.contains("gettablefileurl") ||
           ...
           url.contains("storage.googleapis.com")){
            if(!hasAccess()){
                runOnUiThread(this::mostrarBloqueoPorExpiracion);
                return true;
            }
        ...
        if(!url.contains("appsheet.com") && !url.contains("google.com")){
            if(url.startsWith("http") && !url.contains("datastudio.google.com") && !url.contains("lookerstudio.google.com")){
                ...
                runOnUiThread(() -> mostrarLinkEnVisor(url));
                return true;
            }
        }
        ...
        if(pdfOverlay.getVisibility()==View.VISIBLE){
            runOnUiThread(() -> pdfOverlay.setVisibility(View.GONE));
        }
        return false;
    }
